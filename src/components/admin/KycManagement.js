    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl shadow-2xl flex flex-col rounded-t-2xl" style={{ maxHeight: "90vh", maxHeight: "90dvh" }}>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <h2 className="text-[16px] font-bold" style={{ color: "#1F4E46" }}>Vérification KYC</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={row.label} className="bg-slate-50 rounded-xl p-3">
                ...
                <div className="text-[12px] font-semibold" style={{ color: "#0F172B" }}>{row.value}</div>
              </div>
            ))}
          </div>
          ...
          {(request.documentFront ?? request.documentUrl ?? request.frontImage) && (
            <div>
              <p className="text-[11px] text-slate-400 mb-2 font-semibold uppercase tracking-wider">Documents soumis</p>
          ...
          {error && <p className="text-[12px] text-red-500 bg-red-50 px-3 py-2 rounded-xl">{error}</p>}
        ...
        {request.status === "PENDING" && (
          <div className="shrink-0 px-5 pb-5 pt-3 border-t border-slate-100 flex flex-col gap-2">
            {!rejectMode ? (
              <div className="flex gap-2">
                <button type="button" disabled={loading} onClick={() => submit("APPROVED")}
                  className="flex-1 py-3 rounded-xl text-[13px] font-bold bg-green-500 text-white hover:brightness-110 transition cursor-pointer disabled:opacity-60">
                  {loading ? "Envoi…" : "✓ Approuver"}
                </button>
                <button type="button" disabled={loading} onClick={() => setRejectMode(true)}
                  className="flex-1 py-3 rounded-xl text-[13px] font-bold border border-red-200 text-red-500 hover:bg-red-50 transition cursor-pointer disabled:opacity-60">
                  ✕ Rejeter
                </button>
              </div>
            ) : (
              <div className="flex gap-2">
                <button type="button" disabled={loading} onClick={() => { setRejectMode(false); setReason(""); setError(""); }}
                  className="flex-1 py-3 rounded-xl text-[13px] border border-slate-200 text-slate-500 hover:bg-slate-50 transition cursor-pointer disabled:opacity-60">
                  Annuler
                </button>
                <button type="button" disabled={loading} onClick={() => submit("REJECTED")}
                  className="flex-1 py-3 rounded-xl text-[13px] font-bold bg-red-500 text-white hover:brightness-110 transition cursor-pointer disabled:opacity-60">
